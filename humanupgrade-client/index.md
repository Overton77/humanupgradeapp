...
8. **Auth = Clerk.** No NextAuth, no `_app.tsx`, no pages router, no `<SignedIn>` / `<SignedOut>` (use `<Show when="signed-in">`). `clerkMiddleware()` lives in `proxy.ts` (NOT `middleware.ts` — Next.js 15+ rename).
